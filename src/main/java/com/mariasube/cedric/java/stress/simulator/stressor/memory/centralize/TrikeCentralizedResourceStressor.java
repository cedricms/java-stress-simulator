...
@Slf4j
public class TrikeCentralizedResourceStressor extends AbstractStressor {
    ...
    public void stress() {
        for (int i = 0; i < MAX_STRESSES; i++) {
            try {
                ...
                long numberOfTrikes = getNumberOfTrikes(vehicles);
    ...
    public long getNumberOfTrikes(List<DefaultVehicle> vehicles) {
        return vehicles.stream().filter(vehicle -> "Trike".equals(vehicle.getType())).count();
    }
}
