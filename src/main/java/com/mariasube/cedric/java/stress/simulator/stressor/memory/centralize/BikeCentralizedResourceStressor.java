...
@Slf4j
public class BikeCentralizedResourceStressor extends AbstractStressor {
    ...
    public void stress() {
        for (int i = 0; i < MAX_STRESSES; i++) {
            try {
                ...
                long numberOfBikes = getNumberOfBikes(vehicles);
    ...
    public long getNumberOfBikes(List<DefaultVehicle> vehicles) {
        return vehicles.stream().filter(vehicle -> "Bike".equals(vehicle.getType())).count();
    }
}
